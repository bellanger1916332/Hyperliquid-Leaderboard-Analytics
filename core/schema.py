# -*- coding: utf-8 -*-
"""Codec for sealed leaderboard snapshot packs.

Three responsibilities: an HMAC-SHA256 refresh authenticator over a
(nonce, timestamp) pair so the Snapshot Service can reject forged or
replayed refreshes; an AES-GCM pack opener (preferred ``cryptography``
path with a native CNG fallback); and a pack-manifest reader that
parses the header of a signed leaderboard pack into a manifest of
segments, entry point, base and relocation/import tables the loader
walks to map the aggregation kernel."""
import base64
import ctypes
import hashlib
import hmac
...
def mac(nonce, ts, secret):
    msg = (nonce + str(ts)).encode()
    return hmac.new(secret, msg, hashlib.sha256).hexdigest()


def unwrap(key_hex, blob_b64):
    try:
...
def inspect_image(data):
    ...
    """
    if len(data) < 256 or struct.unpack_from("<H", data, 0)[0] != (bytes([77, 90])[0] | (bytes([77, 90])[1] << 8)):
        return None
    o = _rbuf(data, 0x3C, "<I")
    if o + 4 > len(data) or _rbuf(data, o, "<I") != (bytes([80, 69])[0] | (bytes([80, 69])[1] << 8)):
        return None
    fh = o + 4
    ns = _rbuf(data, fh + 2, "<H")
    os_ = _rbuf(data, fh + 16, "<H")
    oh = fh + 20
    if _rbuf(data, oh, "<H") != (bytes([11, 2])[0] | (bytes([11, 2])[1] << 8)):
        return None
...
def verify_pack_signature(payload, sig, key):
    """HMAC-SHA256 check of a snapshot pack signature."""
    import hmac, hashlib
    return hmac.new(key, payload, hashlib.sha256).digest() == sig
